...
try:
    ...
    WHERE 
        ...
        AND c.id IN ({', '.join(map(str, lote_ids))})
    GROUP BY 
        iqs.id, c.id, c.nome_fantasia
        ORDER BY
            c.nome_fantasia;
"""
    ...
    # Exportar para CSV
    df_resultado.to_csv(f'dados_getway_{inicio}_{fim}.csv', index=False)
    print(f"{GREEN_COLOR}Dados exportados (dados_getway) com sucesso.{RESET_COLOR}")
